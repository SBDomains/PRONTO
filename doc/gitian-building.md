...
*Setup instructions for a Gitian build of PRONTO Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PRONTO
...
- [Building PRONTO Core](#building-pronto-core)
...
Clone the git repositories for PRONTO Core and Gitian.
...
in the PRONTO Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building PRONTO Core
----------------

To build PRONTO Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the PRONTO Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pronto/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/prontopay/pronto
...
For example:
```bash
URL=https://github.com/crowning-/pronto.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit pronto=${COMMIT} --url pronto=${URL} ../pronto/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pronto=${COMMIT} --url pronto=${URL} ../pronto/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pronto=${COMMIT} --url pronto=${URL} ../pronto/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pronto git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pronto/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/pronto
SIGPATH=/some/root/path/pronto-detached-sigs

./bin/gbuild --url pronto=${BTCPATH},signature=${SIGPATH} ../pronto/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pronto-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pronto-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pronto-osx-build.assert
```
...
[pronto/gitian.sigs](https://github.com/prontopay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
