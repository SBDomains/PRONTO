...
const char * const BITCOIN_CONF_FILENAME = "pronto.conf";
const char * const BITCOIN_PID_FILENAME = "prontod.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "pronto" is a composite category enabling all PRONTO-related debug output
                if(ptrCategory->count(std::string("pronto"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PRONTO
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PRONTO
    // Mac: ~/Library/Application Support/PRONTO
    // Unix: ~/.pronto
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PRONTO";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PRONTO";
#else
    // Unix
    return pathRet / ".pronto";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty pronto.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override pronto.conf
